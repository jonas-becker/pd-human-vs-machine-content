...
def semantic_sim_bert(df, pair_ids_train, pair_ids_test, text1_train, text1_test, text2_train, text2_test, y_train, y_test):
    ...
    print("Wheighting classes: " + str(class_weight))
    ...
    clf = make_pipeline(StandardScaler(),
                        SGDClassifier(max_iter=1000, class_weight={False: class_weight[0], True: class_weight[1]}, verbose=True)
                        )
    # train the model
    ...
    prediction_result = y_pred.tolist()
...
for dataset in datasets_in_df:
    ...
    df_dataset[SUPPLEMENT_PAIR] = False
    ...
    if False not in df_dataset[PARAPHRASE].unique():
        ...
        supplement_df[SUPPLEMENT_PAIR] = True
    ...
    feature_pairs = [[x] + [df_dataset[TEXT1].iloc[i]] + [df_dataset[TEXT2].iloc[i]] + [df_dataset[PARAPHRASE].iloc[i]] for i, x in tqdm(enumerate(feature_pairs), total=len(feature_pairs))]
    ...
    for train_index, test_index in sss.split(X, y):
        ...
        train_data[3] = train_data[3] + [True if x == "True" else False for x in X[train_index][:, 3].tolist()]     # labels
        ...
        print("sdfjk: " + str(len(pred_result_df[PAIR_ID])))

# use classifiers
print("Finished assembling data. Continue with classification...")
pred_result_df[SEM_BERT] = semantic_sim_bert(df, train_data[0], test_data[0], train_data[1], test_data[1], train_data[2], test_data[2], train_data[3], test_data[3])
...
print(len(pred_result_df))
pred_result_df.to_json(os.path.join(OUT_DIR, DETECTION_FOLDER, "detection_test_result.json"), orient="index", index=True, indent=4)
